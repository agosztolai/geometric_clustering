...
class GeoCluster(object): 
    ...
    def plot_graph(self, t, node_size=50, edge_width=5, node_labels=False, cluster=False):
        ...
        edge_cmap = plt.cm.ScalarMappable(
            norm = plt.cm.colors.Normalize(edge_vmin, edge_vmax),
            cmap = plt.cm.coolwarm)
        fig.colorbar(edge_cmap, label='Edge curvature')

        nodes_colorbar = nx.draw_networkx_nodes(self.G, pos=pos, node_size=node_size, node_color=_labels, cmap=plt.get_cmap("tab20"))
    ...
    def plot_graph_3D(G, node_colors=[], edge_colors=[], params=None, save=False, ext='.png'):
    ...
    def plot_embedding(self, folder = 'images', ext='.png'):
        ...
        for i in tqdm(range(len(self.Y))):
            ...
            plt.axis('tight')
            plt.savefig(os.path.join(folder, 'image_' + str(i) + ext))
            plt.close()

...
def plot_edge_curvatures(times, kappas, ylog=False, filename='edge_curvature', ext='.svg'):
...
def plot_graph_snapshots(graph, times, kappas, folder='images', filename = 'image', 
                         node_size=20, edge_width=2, node_labels=False, 
    ...
    for i, kappa in tqdm(enumerate(kappas), total=len(kappas)):
        plot_graph(graph, kappa, node_size=node_size, node_labels=node_labels, edge_width=edge_width, figsize=figsize)
        plt.title(r'$log_{10}(t)=$' + str(np.around(np.log10(times[i]),2)))
        plt.savefig(folder + '/' +  filename + '_%03d' % i + ext, bbox_inches='tight')
        plt.close()


def plot_graph(graph, kappa, node_size=20, edge_width=1, node_labels=False, 
               node_colors=None, figsize=(10, 7)):
    ...
    if node_colors is None:
        B = nx.incidence_matrix(graph, weight='weight').toarray()
        inverse_degrees = np.diag([1./graph.degree[i] for i in graph.nodes])
        node_colors = inverse_degrees.dot(B.dot(kappa))
    ...
    nx.draw_networkx_nodes(graph, pos=pos, node_size=node_size, node_color=node_colors, cmap=cmap, vmin=vmin, vmax=vmax) 
    nx.draw_networkx_edges(graph, pos=pos, width=edge_width, edge_color=kappa, 
                           edge_vmin=edge_vmin, edge_vmax=edge_vmax, edge_cmap=edge_cmap)
